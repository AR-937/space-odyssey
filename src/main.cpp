...
std::vector<Star> createStars(uint32_t count, float scale) {
    ...
    sf::Vector2f const window_world_size = conf::windows_size_f * conf::near;
    sf::FloatRect const star_free_zone = {-window_world_size * 0.5f, window_world_size};

    // Create randomly distributed stars
    for (uint32_t i{count}; i--;) {
        float const x = (dis(gen) - 0.5f) * conf::windows_size_f.x * scale;
        float const y = (dis(gen) - 0.5f) * conf::windows_size_f.y * scale;
        float const z = dis(gen) * (conf::far - conf::near) + conf::near;

        // Remove stars in the star free zone
        if (star_free_zone.contains(x, y)) {
            ++i;
            continue;
        }

        // Else add star in the vector
        stars.push_back({{x, y}, z});
    }
    // Depth ordering
    std::sort(stars.begin(), stars.end(), [](Star const &s_1, Star const &s_2) { return s_1.z > s_2.z; });
...
int main() {
    auto window = sf::RenderWindow({conf::windows_size.x, conf::windows_size.y}, "Stars", sf::Style::Fullscreen);
    ...
    while (window.isOpen()) {
        ...
        for (uint32_t i{conf::count}; i--;) {
            ...
            if (s.z < conf::near) {
                // Sends stars moving across camera to the back
                s.z = conf::far - (conf::near - s.z);
                // FIFO repeat pattern for stars
                first = i;
            }
        }
        ...
        states.transform.translate(conf::windows_size_f * 0.5f);
